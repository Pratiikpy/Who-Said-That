...
function InboxIcon({ active }: { active: boolean }) {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? "#FAFAFA" : "#71717A"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
export function BottomTabs({ hidden }: { hidden?: boolean }) {
        ...
        {tabs.map((tab) => {
          ...
          if (tab.isCompose) {
            return (
              ...
              >
                <div
                  className="w-[48px] h-[48px] rounded-2xl flex items-center justify-center active:scale-90 transition-transform"
                  style={{
          ...
          return (
            <Link
              ...
              href={tab.href}
              onClick={() => select()}
              className="relative flex flex-col items-center pt-2 pb-1.5 min-w-[56px] min-h-[48px] active:opacity-70 transition-opacity"
              style={{ WebkitTapHighlightColor: "transparent" }}
            >
              {/* Active indicator bar */}
              {active && (
                <div className="absolute top-0 w-8 h-[2px] rounded-full bg-accent" />
              )}
